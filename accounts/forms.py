...
from app.models import SolicitudMedico
...
class PerfilForm(forms.ModelForm):
    ...
    def clean_first_name(self):
        ...
        return first_name 
class SolicitudMedicoForm(forms.ModelForm):
    """Formulario para solicitar el rol de médico."""
    class Meta:
        model = SolicitudMedico
    ...
    def clean_matricula(self):
        """Validación personalizada: matrícula única."""
        matricula = self.cleaned_data.get('matricula', '').strip()
        
        if not matricula:
            raise forms.ValidationError("La matrícula es obligatoria.")
        
        # Verificar que no exista ya en otro médico
        from app.models import Medico
        if Medico.objects.filter(matricula=matricula).exists():
            raise forms.ValidationError("Ya existe un médico con esa matrícula.")
        
        # Verificar que no exista otra solicitud pendiente con la misma matrícula
        qs = SolicitudMedico.objects.filter(matricula=matricula, estado='PENDIENTE')
        if self.instance and self.instance.pk:
            qs = qs.exclude(pk=self.instance.pk)
        if qs.exists():
